...
| Notes | Weekly minutes and live meeting notes both use `DocsEditor`, so meeting mode is one normal flowing document instead of the retired block/column editor. The old block modules remain only for backwards-format tests and are no longer reachable from meeting mode. |
...
| Material theme builder opened to everyone | 2026-08-20. The hidden 19-click Boss colophon-logo unlock and one-account gate were retired at the user's request. The advanced Material builder now appears directly in the shared ThemeSwitcher used by both Summer and Boss styles. Its manual Primary/Secondary colour inputs and summary swatches were removed; a new personal theme must come from the wallpaper/photo Material seed and keeps its own light/dark mode. Named Material themes still save through the existing per-user preference sync, while the legacy primary/secondary loading branch remains so older saved themes are not stranded. The footer logo is a plain image again, preference sync no longer clears custom colour state for non-owner accounts, and the now-unreferenced `custom-color-access.js` helper was deleted. |
| Full theme catalogue restored | 2026-08-19, same day, explicit follow-up request. `SELECTABLE_THEME_KEYS` had been narrowed to `["default"]` (comment: "MABIS, and nothing else (Novesce, Aug 2026)") — a separate, much broader gate than `canUseCustomColors()` above: it hid the entire curated catalogue from every visitor, not just the "make your own colors" surface, including all 140 themes (15 BFDI colourways, the pride flags, GMK keysets, Touhou, Sonic, Nintendo consoles, Catppuccin, the pastels, Frutiger Aero). Changed to `Object.keys(THEMES)` so every theme in the catalogue is selectable again, and nothing added to `THEMES` in future can go stale here by omission the way an explicit list did. Verified: `check-theme-balance.mjs` — all 140 themes still pass semantic hue, role, editor-palette, slot and contrast checks (unaffected; that script always read `THEMES` directly, never `SELECTABLE_THEME_KEYS`); `check:bundle` budgets still pass — the catalogue is lazy behind ThemeSwitcher/SettingsModal, so restoring it does not touch the boot chunk; both builds and full `check:solid:parity` green. |
| Document ink/highlight colours were frozen, not theme-relative | 2026-08-19. `applyThemeInk`/`applyThemeHighlight` (`DocsEditor.jsx`) wrote two things per formatted span: the theme-relative `themeInk`/`themeHighlight` CLASS (correctly tied to `hsl(var(--editor-ink-*))` in `index.css`), and — "belt and braces" — ALSO a literal inline `color`/`background` computed via `resolveThemeColor()` at the moment of the click. An inline style always beats a class selector regardless of source order, and the class rules were not `!important`, so the frozen snapshot always won and never updated again on a theme switch — text coloured while the MABIS theme was active stayed that literal red under every other theme forever, both live and once saved. Two-part fix, because a JS-only fix cannot touch documents already saved with the stale inline colour baked in: (1) `index.css`'s `.theme-rich-text .ql-ink-*`/`.ql-hl-*` rules gained `!important`, so the theme-token class wins over ANY stray inline colour — old documents render correctly under today's theme retroactively, no data migration; (2) `applyThemeInk`/`applyThemeHighlight` no longer write the redundant inline colour at all, so newly-applied ink/highlight only ever carries the class going forward. `resolveThemeColor` import removed from `DocsEditor.jsx` as dead (nothing else in the codebase calls it; left exported from `quill-setup.js` as a generically reasonable utility). Verified: `check-theme-balance.mjs` still 140/140 (the ink/highlight tokens themselves weren't touched, only how they're applied and enforced), both builds, full `check:solid:parity` green. |
| Boss-themed catalogue gated by 69 Boss-style activations | 2026-08-20, explicit clarification. The large LGBTQ+, BFDI, Touhou, Linux, game-console, character, GMK, and related palette catalogue is now available only after a person activates the real Boss style button 69 times. The completed unlock persists through the existing `mabis-boss-themes-unlocked` preference and notifies an already-mounted ThemeSwitcher through `bossThemesUnlocked`. Removed the hardcoded account ID/email bypass, so every person goes through the same gesture. Boss style itself and the Material You wallpaper/photo builder remain directly available. The design contract now pins the count, storage gate, reactive event, and absence of an identity bypass. |
| First signed-in style choice | 2026-08-20. Home waits for account preference pull and opens a required two-option chooser only when no explicit valid `mabis-home-layout` exists. Summer is described as simple and familiar; Boss as modern and editorial. Each option includes opt-in Japanese companion copy and writes through `setHomeLayout()`, so even Summer becomes an explicit account-synced choice and a saved choice does not prompt again. The chooser does not touch the separate 69-press Boss-theme catalogue counter. |
| Saved custom theme before first paint | 2026-08-20. The final theme snapshot was already correct, but `applyThemeSnapshot()` ran in the external module after `solid/index.html` had painted a hard-coded dark loading surface. A tiny blocking head replay now validates the saved theme/font identity and applies the cached CSS variables before critical CSS; an immediate body handoff restores owned polarity/theme classes, and the loading background, text, progress accent and browser `theme-color` consume the saved semantic tokens. Both Material You seeds and legacy custom pairs use the same path. `check-theme-prepaint.mjs` executes the shipped HTML against matching and stale snapshots (9/9). |
| Loader readiness and scroll-lock hardening | 2026-08-20. LoadingScreen now owns the shared scroll lock for its full mount and blocks overscroll/touch chaining. The lock manager covers both `html` and `body`, exposes an active class, restores both original values after nested releases, and still invalidates stale HMR/navigation releases. On capable links, the ten numbered Home section modules start while auth is in flight and their real `Promise.allSettled` completion is awaited before the loader exits; the 70 KiB document editor and below-fold reads remain deferred, and constrained/Save-Data links retain the smaller first-view path. |
| Live loading theme tokens | 2026-08-21. The Solid loading overlay no longer paints fixed editorial `ink`/`bone` roles. Its surface, copy, grid, primary progress and secondary glow consume the same pre-painted semantic tokens as the restored custom or Material theme, including the initial HTML splash handoff. |
| Keyboard and controller navigation | 2026-08-21. Arrow keys move spatial focus outside inputs/editors. A hidden Gamepad API loop shares the same engine for standard PS5, Xbox and Nintendo mappings and raw GameCube USB-adapter axes/buttons; A activates and B closes/goes back. No controller prompt or visible setting was added. |
| Document editing repairs | 2026-08-21. The toolbar is sticky; external paste strips foreign color/background paint while preserving links, lists and emphasis; link insertion retains the selected range and supports opening; plain built-in MABIS alone exposes native unrestricted color pickers with contrast-checked highlight ink, while saved custom, Material and catalogue themes fall back to live semantic document roles. Meeting mode now uses the normal Quill document instead of block notes. |
| Linux quality-first profile | 2026-08-21. Linux is marked before mount and never enters the visual-reduction `performance-lite` tier. Full motion, glass, grain and cursor effects remain; performance comes from the existing refresh-aware physics, BORE-inspired cooperative scheduler, containment and narrowly-scoped compositor hints. Browsers expose no safe direct kernel scheduler/Vulkan surface to page code, so none is fabricated. |
