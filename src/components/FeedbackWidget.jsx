            ...
            <AnimatePresence mode="wait">
              {submitted ? (
                <motion.div key="success"
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Your {tab === "feedback" ? "feedback" : "bug report"} has been submitted.</p>
                </motion.div>
              ) : (
                <motion.div key="form"
                  ...
                  <div className="flex gap-1 mb-4 p-1 bg-muted rounded-lg">
                    <motion.button whileTap={{ scale: 0.95 }} onClick={() => setTab("feedback")}
                      className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-colors ${tab === "feedback" ? "bg-card text-primary shadow-sm" : "text-muted-foreground"}`}>
                      Feedback
                    </motion.button>
                    <motion.button whileTap={{ scale: 0.95 }} onClick={() => setTab("bug")}
                      className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-colors ${tab === "bug" ? "bg-card text-primary shadow-sm" : "text-muted-foreground"}`}>
                      Report Issue or Bug
                    </motion.button>
                  </div>
                  ...
                  {tab === "feedback" && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mb-4 overflow-hidden">
                      <label className="text-xs font-bold text-muted-foreground mb-2 block">
                        Satisfaction: <span className="text-primary">{rating}/10</span>
                      </label>
                  ...
                  <div className="mb-4">
                    ...
                    <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={4}
                      ...
                      className="w-full rounded-lg border border-border px-3 py-2 text-sm focus:outline-none focus:border-primary/50 resize-none" />
                    {imageUrl ? (
                      ...
                      <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading}
                        className="mt-2 flex items-center gap-1.5 text-xs font-semibold text-primary hover:bg-primary/5 px-3 py-1.5 rounded-lg border border-primary/30 transition-colors disabled:opacity-50">
