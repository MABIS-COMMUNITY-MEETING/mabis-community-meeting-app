      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-x-3 top-20 z-[61] max-h-[calc(100dvh-6rem)] w-auto overflow-y-auto rounded-2xl border border-border bg-card p-4 shadow-xl sm:inset-x-auto sm:right-6 sm:top-24 sm:max-h-[75vh] sm:w-80"
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <Palette className="w-4 h-4 text-primary" />
                <p className="text-sm font-bold text-foreground">Customize Profile Picture</p>
              </div>
            ...
            <motion.label
              ...
              whileTap={{ scale: 0.98 }}
              className="flex items-center gap-2 cursor-pointer mb-3 bg-muted hover:bg-muted rounded-lg px-3 py-2.5 border border-border transition-colors"
            >
            ...
            <p className="text-[10px] text-muted-foreground mb-2">Profile picture:</p>
            <div className="flex gap-2 mb-3">
              <motion.button
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
                onClick={async () => { await base44.auth.updateMe({ avatar_url: null }); syncAvatarToMember({ avatar_url: null }); updateUser?.(); }}
                className={`w-12 h-12 rounded-full overflow-hidden border-2 transition-transform bg-card flex items-center justify-center ${!user?.avatar_url ? "border-primary scale-110" : "border-border"}`}
                title="MABIS Logo"
              >
            ...
            {avatarHistory.length > 0 && (
              <div className="mb-3">
                <div className="flex items-center gap-1 mb-2">
                  ...
                  <p className="text-[10px] text-muted-foreground">Recent profile pictures ({avatarHistory.length}):</p>
                </div>
            ...
            <div className="flex gap-2">
              <button onClick={handleReset}
                className="flex-1 text-[10px] text-muted-foreground hover:text-muted-foreground underline text-center">
                Reset to default
              </button>
            </div>
          </motion.div>
