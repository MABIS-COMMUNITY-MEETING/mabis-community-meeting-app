/*
 * Adapted from dashersw/liquid-glass-js/container.js.
 * Copyright (c) 2025 Armagan Amcalar. Licensed under the MIT License.
 * Source: https://github.com/dashersw/liquid-glass-js
 *
 * MABIS adaptations: ES module export, use an existing Solid-owned element,
 * resize observation, requestAnimationFrame scroll throttling, ready state,
 * and deterministic teardown. The WebGL shader and optical parameters remain
 * from the upstream Container implementation.
 */
...
export class LiquidGlassContainer {
  ...
  constructor(options = {}) {
    ...
    // Add to instances
    LiquidGlassContainer.instances.push(this)

    // Initialize
    this.init()
    if (typeof ResizeObserver !== 'undefined' && this.element) {
      this.resizeObserver = new ResizeObserver(() => this.updateSizeFromDOM())
  ...
  init() {
    ...
    if (LiquidGlassContainer.pageSnapshot) {
      // Snapshot already exists, initialize immediately
      this.initWebGL()
    } else if (LiquidGlassContainer.isCapturing) {
      // Snapshot in progress, add to waiting queue
      LiquidGlassContainer.waitingForSnapshot.push(this)
    } else {
      // Start snapshot process
      LiquidGlassContainer.isCapturing = true
      LiquidGlassContainer.waitingForSnapshot.push(this)
  ...
  createElement() {
    // MABIS supplies the existing Solid-owned surface so menus, focus and
    // semantics stay in the component tree; the upstream-created element is
    // retained as a fallback for standalone use.
    ...
    this.canvas.className = 'lg-liquid-canvas'
  ...
  capturePageSnapshot() {
    ...
    html2canvas(document.body, {
      ...
      .then(snapshot => {
        ...
        const waitingContainers = LiquidGlassContainer.waitingForSnapshot.slice()
  ...
  `
    ...
    void main() {
        ...
        // LiquidGlassContainer position in viewport coordinates
  ...
  startRenderLoop() {
    ...
    // Store lifecycle hooks for the Solid wrapper.
    this.render = render
    ...
    this.element.classList.add('lg-webgl-ready')
    this.element.dispatchEvent(new CustomEvent('liquidglassready'))
  }
  ...
  destroy() {
    ...
    if (index >= 0) LiquidGlassContainer.instances.splice(index, 1)
