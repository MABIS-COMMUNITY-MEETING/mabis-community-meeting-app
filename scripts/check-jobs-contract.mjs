...
assert.match(jobsSource, /base44\.entities\.Member\.update\(member\.id, update\)/);
assert.match(memberSchemaSource, /"job_rotation_excluded_week"/);
assert.match(jobsSource, /studentMembers\(\)\.filter\(\(m\) => participatesInJobs\(m, currentWeek\(\)\)\)/);
assert.match(jobsSource, /data-cursor-lite/);
assert.match(jobsSource, /onDelete=\{handleRemoveAssignment\}/);
assert.match(jobsSource, /onJobStatus=\{handleJobStatus\}/);
assert.match(jobTablesSource, /Check off job/);
assert.match(jobTablesSource, /props\.onJobStatus\(props\.assignment/);
