...
- Off-screen Home widgets are skipped by `LazySection`, which mounts on approach, never unmounts, and reserves the gap with that section's own `contain-intrinsic-size`. Do **not** add `content-visibility: auto` to the section wrapper on top of it: a skipped element reports its intrinsic-size estimate rather than its real height, so a flat guess for every section changes the document height as each one renders and the scrollbar jumps under the reader. Skip-rendering belongs where the height is known — the placeholder — not on the wrapper. `check:performance` fails if it returns.
...
- During an active gesture, `html.is-scrolling` drops `backdrop-filter` on glass surfaces to a flat translucent fill, hides the full-viewport grain layer, and pauses decorative infinite animations; all of it returns ~140 ms after scroll settles. These are per-frame GPU costs the reader cannot perceive while the page is moving, and paying them is what makes a native scroll stutter on an integrated GPU. Document height is cached between resize notifications. Only `performance-lite` and unsupported-browser fallbacks may replace the glass backdrop with a permanently static translucent surface.
