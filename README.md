...
The app renders in one of two presentations, chosen per user under **Settings → Page Layout** and stored in `src/lib/layout-preference.js`. This is a deliberate exception to the rest of this document, requested directly by Novesce: the default **is** the editorial system, with Summer style as the opt-in exception.
...
- every route still reachable (the Summer bar carries a small **Pages** menu in place of the editorial overlay);
...
- `solid/components/home/shell.jsx` (boss, default)
- `solid/components/home/summer.jsx` and `SummerHeader.jsx` (summer, opt-in)
