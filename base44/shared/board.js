// Shared plain-text rendering of the community board.
// Used by the read-only textView endpoint and the interactive CLI API.

export function line(char = '─', n = 62) { return char.repeat(n); }
...
export function wrap(text, width = 62, indent = '  ') {
...
export async function buildBoardText(sr) {
  ...
  for (const week of Object.keys(byWeek).sort().reverse().slice(0, 2)) {
    ...
    for (const a of byWeek[week]) {
      ...
      out.push(`    ${String(a.job_title).padEnd(26)} ${String(a.assigned_to_name || '?').padEnd(18)} done:${done} missed:${notDone}`);
    }
  }
  out.push('');
